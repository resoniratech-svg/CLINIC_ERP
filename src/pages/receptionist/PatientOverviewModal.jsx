    ...
    >
      {loading ? (
        ...
        <div className="space-y-6 text-xs text-slate-700">
          ...
          {/* Previous Visit & Follow-up Readiness Status */}
          {data?.previous_visit_status && (
            <div className={`p-4 rounded-2xl border ${
              data.previous_visit_status.is_ready_for_followup
                ? 'bg-emerald-50/60 border-emerald-200'
                : !data.previous_visit_status.has_previous_visit
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <span className="text-[11px] font-bold uppercase tracking-wider text-slate-700 flex items-center gap-1.5">
                    ...
                    <span>Previous Visit & Follow-Up Readiness</span>
                  </span>
                  {data.previous_visit_status.appointment_date && (
                    <span className="text-[10px] text-slate-500 font-mono">
                      (Visit Date: {String(data.previous_visit_status.appointment_date).slice(0, 10)})
                    </span>
                  )}
                </div>
                ...
                <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold inline-flex items-center gap-1 ${
                  ...
                  {data.previous_visit_status.is_ready_for_followup ? (
                    ...
                    <span>New Patient (No Previous Visits)</span>
                  ) : (
                    <>
                      <AlertCircle className="w-3 h-3 text-amber-600" />
                      <span>Previous Visit Incomplete</span>
                    </>
                  )}
                </span>
              </div>

              {data.previous_visit_status.pending_reason && (
                <div className="text-[11px] p-2.5 bg-amber-100/70 border border-amber-200 rounded-xl text-amber-900 font-medium flex items-center gap-2">
                  <AlertCircle className="w-4 h-4 text-amber-700 shrink-0" />
                  <span>{data.previous_visit_status.pending_reason}</span>
                </div>
              )}

              {/* Step progression chips */}
              {data.previous_visit_status.steps && (
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 pt-1">
                  {data.previous_visit_status.steps.map((step, sIdx) => (
                    <div
                      key={sIdx}
                      className={`p-2 rounded-xl border text-center text-[10px] ${
                        step.completed
                    ...
                    >
                      ...
                      <div className="mt-0.5 flex items-center justify-center gap-1 font-semibold uppercase tracking-wider text-[9px]">
                        {step.completed ? (
                          <>
