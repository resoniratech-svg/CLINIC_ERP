...
async function setDoctorTarget(req, res) {
  try {
    ...
    `, [
      ...
      enquiry_target || 0, unit_target || 0, referral_target || 0, revenue_target || 0
    ]);

    res.locals.auditEntry = { module: 'Target Management', action: 'Set Doctor Target', recordId: result.rows[0].id, newValue: result.rows[0] };
    return res.status(201).json(formatResponse(true, result.rows[0], 'Doctor target set successfully'));
  } catch (err) {
...
async function getDoctorPerformance(req, res) {
  try {
    ...
    const query = `
      ...
      WHERE d.branch_id = $1
      GROUP BY d.doctor_id, u.full_name, d.specialization, dt.enquiry_target, dt.unit_target, dt.referral_target, dt.revenue_target
    `;
