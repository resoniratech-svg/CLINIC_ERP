...
export const ReceptionistLayout = () => {
  ...
  // Allow receptionist or super_admin
  if (!token || !user) {
    return <Navigate to="/login" replace />;
  }

  const isAuthorized = user.role === 'receptionist' || user.role === 'super_admin';
  if (!isAuthorized) {
    return <Navigate to="/login" replace />;
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50/70 flex flex-col">
      ...
      <div className="lg:pl-64 flex flex-col flex-1 min-w-0">
        <ReceptionistHeader onToggleMobile={() => setIsMobileOpen(!isMobileOpen)} />
